-- Migration: 20260210190000_cleanup_functions.sql
-- Description: Nettoyage des anciennes fonctions pour éviter la confusion

-- Supprimer toutes les anciennes versions de la fonction complete_mission_with_fuel
...
BEGIN
    ...
    IF p_montant > 0 AND p_is_charge_entreprise THEN
        BEGIN
            INSERT INTO public.tb_depenses (
                ...
                v_chauffeur_id,
                p_mission_id,
                COALESCE(p_raison, 'Course'), -- Valeur par défaut pour la catégorie
                p_montant,
                p_devise,
    ...
    BEGIN
        INSERT INTO public.tb_journal_bord (
            ...
            v_chauffeur_id,
            p_mission_id,
            'Fin de mission', -- Valeur par défaut
...
-- Vérifier qu'il n'y ait qu'une seule version de la fonction souhaitée
DO $$
DECLARE
    func_count INTEGER;
BEGIN
    ...
    WHERE n.nspname = 'public' AND p.proname = 'complete_mission_simple';
    
    IF func_count != 1 THEN
        RAISE WARNING 'ATTENTION: Il y a % versions de la fonction complete_mission_simple.', func_count;
    ELSE
        RAISE NOTICE 'Fonction complete_mission_simple correctement configurée.';
    END IF;
END $$;